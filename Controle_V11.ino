...
float erro_PH = 0.0; // Variável para armazenar o cálculo do erro entre SP e valor lido do sensor
float kP_PH = 1.0; // Valor do ganho proporcional
float kI_PH = 0.0; // Valor do ganho integral
float I_PH = 0.0; // Valor da ação integral acumulativa
float PI_PH = 0.0; // Valor da ação proporcional integral
float P_PH = 0.0; // Valor da ação proporcional
float pulsos_PH1 = 0.00003; // Relação entre número de pulsos da bomba peristática e aumento no pH
int bomba_PH1 = 0; // Varíavel que irá armazenar a quantidade de pulsos enviados para a bomba
float pulsos_PH2 = 0.00001525; // Relação entre número de pulsos da bomba peristática e decremento no pH
float bomba_PH2 = 0; // Varíavel que irá armazenar a quantidade de pulsos enviados para a bomba
int hora_atual = 0; // Hora atual que será atualizada conforme a hora do Indusoft
int atualiza_hora = 0; // Variável para controlar a quantidade de vezes que o controle é executado
int y1 = 0; // Contador para bomba de recirculação
...
// Variáveis de temperatura da aquaponia
int SP_TEMP_AQP = 0;
int PV_TEMP_AQP = 0;


/*--------------------INSTANCIAMENT DOS OBJETOS--------------------*/
...
void loop()
{
  Mb.Run();
    ...
    Mb.R[1]; // PV da temperatura da aquaponia
    Mb.R[2]; // SP da temperatura da aquaponia 
...
/*--------------------RECEBIMENTO DE DADOS DO INDUSOFT--------------------*/
    ...
    //  Mb.R[71] : Horário para controle automático do pH
    //  Mb.R[72] : Status do controle de temperatura da aquaponia
       ...
       if (tempo_atual - ut_ph >= 10000 || tempo_atual < 90000)
        {
         ...
         measure = analogRead(A0); //Leitura do sensor de pH
         lastRead2 = (measure + lastRead2) / 2; // Média entre último valor e valor atual
    ...
    /* LEITURA DA LUMINOSIDADE - SENSOR LDR */
     ...
     /* LEITURA DA ALTURA DAS PLANTAS - SENSOR ULTRASONICO */
       ...
       if (tempo_atual - ut_altcult >= 5000 || tempo_atual < 90000)
        ...
        /* LEITURA DOS VALORES DE TEMPERATURA DA AQUAPONIA - SP e MV */

          PV_TEMP_AQP = Mb.R[1]*0.02442;
          SP_TEMP_AQP = Mb.R[2];
...
/*--------------------ACIONAMENTO DOS ATUADORES--------------------*/
   ...
   if ( PV_TEMP_AQP+1 < SP_TEMP_AQP and Mb.R[72] == 1 and PUMP_AQP == 0 )
   {
    PUMP_ON ();
   }
   
   if( PV_TEMP_AQP+1 > SP_TEMP_AQP || Mb.R[72] ==0 and PUMP_AQP ==1)
   {
    PUMP_OFF ();
   }
   ...
   /* Acionamento manual da bomba peristaltica PH1 + */

   if (Mb.R[57] == 1 and x1==0)
   {
    BOMBA_PH1_ON (Mb.R[58]);
   }

   /* Acionamento manual da bomba peristaltica PH2 - */

   if (Mb.R[59] == 1 and x2==0)
   {
    BOMBA_PH2_ON (Mb.R[60]);
   }


   /* Condição que permite apenas uma iteração da rotina de controle do pH por hora */
   
   hora_atual = Mb.R[66]; 
   if(atualiza_hora == 1 and hora_atual != Mb.R[71]) 
   {
    atualiza_hora = 0; 
   }
   
   /* Acionamento do controle automático de pH */
     
   if (Mb.R[64] == 1 and x1==0 and x2 == 0 and atualiza_hora == 0)
   {              
    if (hora_atual == Mb.R[71]) // Às 9h será realizado o controle de pH uma vez
    {
      
      // Recircular água do reservatório
      PUMP_ON();
      while(y1 < 10000)
      {
        y1++;
      ...
      // Não permitir que o controle seja executado mais de uma vez na mesma hora
      atualiza_hora = 1;
      
      setPoint_PHf = Mb.R[65]; // Recebe do indusoft o valor inteiro do SP de pH multiplicado por 100
      setPoint_PH = setPoint_PHf/100; // Obtem o valor do SP com duas casas decimais
      
      erro_PH = setPoint_PH - ((lastread/-72.9927010)+13.946); // Cálculo do erro com a conversão do valor do sensor para pH entre 0 e 14
  
      P_PH = erro_PH * kP_PH; // Ação proporcional
      
      if (P_PH > 0)
      {
        bomba_PH1 = P_PH/pulsos_PH1;
        Serial.print (" Pulsos B1:");
        Serial.print (bomba_PH1);
        Serial.println ("   ");
        if (bomba_PH1 > 50000)
        {
          bomba_PH1 = 50000; // Limitador para que a bomba não gire mais de 50000 pulsos
        }
        BOMBA_PH1_ON(bomba_PH1);  
      }
      if (P_PH < 0)
      {
        bomba_PH2 = (-1.0)*P_PH/pulsos_PH2;
        Serial.print (" Pulsos B2:");
        Serial.print (bomba_PH2);
        Serial.println ("   ");
        if (bomba_PH2 > 50000)
        {
          bomba_PH2 = 50000;
        }
        BOMBA_PH2_ON(bomba_PH2);                 
      }
...
void BOMBA_PH1_ON (int Pulsos)
{
  digitalWrite(ENB_PH, LOW);
  digitalWrite(DIR_PH1, HIGH);
  while ( x1 < Pulsos)
      {
        digitalWrite(PUMP_PH1, HIGH);
        delayMicroseconds(5000);
        digitalWrite(PUMP_PH1, LOW);
        delayMicroseconds(5000);
        x1++;
        Serial.print (" Pulsos B1, x1:");
        Serial.print (x1);
        Serial.println ("   ");
      }
   digitalWrite(ENB_PH, HIGH); 
   digitalWrite(PUMP_PH1, LOW); 
   x1 = 0; 
}

void BOMBA_PH2_ON (float Pulsos)
{
  digitalWrite(ENB_PH, LOW);
  digitalWrite(DIR_PH2, LOW);
  while ( x2 < Pulsos)
      {
        digitalWrite(PUMP_PH2, HIGH);
        delayMicroseconds(5000);
        digitalWrite(PUMP_PH2, LOW);
        delayMicroseconds(5000);
        x2++;
        Serial.print (" Pulsos B2, x2:");
        Serial.print (x2);
        Serial.println ("   ");
      }
  digitalWrite(ENB_PH, HIGH); 
...
{
  Serial.print ("PV:");
  Serial.print (PV_TEMP_AQP);
  Serial.print (" SP:");
  Serial.print (SP_TEMP_AQP);
  Serial.print ("Mb.R[72]:");
  Serial.print (Mb.R[72]);
